...
from frontend.models import reservationdb,sign_indb,transactiondb
...
def savereservation(req):
    if req.method == "POST":
        ...
        obj.save()
        pay=productdb.objects.get(place=dt)
        messages.success(req, "Reserved")

        return render(req,"payment.html",{"pay":pay})
...
def savepayment(req):
    if req.method=="POST":
        ...
        a = req.POST.get('amount')
        obj = transactiondb(cardnumber=cn,monthyear=my,cvvcode=cc,cardname=c,amount=a)
        obj.save()
