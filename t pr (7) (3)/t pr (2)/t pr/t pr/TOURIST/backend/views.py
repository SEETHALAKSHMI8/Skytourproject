...
def saveproduct(req):
    if req.method=="POST":
        ...
        im = req.FILES['image']
        obj=productdb(country=c,place=p, description=d,pricerange=pr,image=im)
        obj.save()
...
def updateproduct(req,dataid):
    if req.method=="POST":
        ...
        productdb.objects.filter(id=dataid).update(country=c, place=p,description=d, pricerange=pr, image=file)
